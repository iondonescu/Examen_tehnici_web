...
function startGame(){
    ...
    sec=0;
    min=0;
    b[0].innerHTML=0;//number of moves
    shuffle(catArray);
    for (let i=0;i<=cards.length-1;i++){
      ulList[i].classList.add(catArray[i]);//add class to each element of ulList
    };
    for (let i=0;i<=cards.length-1;i++){
      cards[i].addEventListener('click',showCard);//add event and toggle class
    };
};
startGame();
...
/*
 * event-clicking the volume button change the icon and class (enable/disable)
 */
