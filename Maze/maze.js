...
let cols, rows; // number of cols and rows of canvas
let w = 40; // dimension of a cell in canvas
let grid = [];// array for each cell
let current;// current object
let stack = [];// stack for backtracking
let cell ; // object created with class Cell
let img; //the player
...
function draw() {
    ...
    //generating a maze, use depth first search algorithms and backtracking
...
//window pops-up when the character reaches the spot
...
//check if there are walls on top, bottom,right and left player, if not you can move the player
    ...
    if ( down === 360 && upp === 360)setTimeout(function(){
        movesNumber.innerHTML= "You've made " + moves + " moves!";
        toggleModal() ;
    }, 1000);
}
